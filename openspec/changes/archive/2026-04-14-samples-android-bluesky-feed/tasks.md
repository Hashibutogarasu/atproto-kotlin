...
- [x] 1.3 Create `samples/android/` directory with an empty `build.gradle.kts` scaffold using the android application plugin, Compose enabled, `compileSdk = 34`, `minSdk = 28`, `namespace = "io.github.kikin81.atproto.samples.bluesky"` _(bumped `compileSdk`/`targetSdk` to `36` because `androidx.core:core-ktx:1.15.0` requires `compileSdk ≥ 35`. `minSdk = 28` as specified.)_
